...
def plot_profiles_3d(inputs, save=False):
    ...
    face_colors_3d = viridis_inv((grid_soc_smth - grid_soc_smth.min()) / (grid_soc_smth.max() - grid_soc_smth.min()))
    ...
    ax.set_zlabel("X-Avg. Temperature [°C]")
    ...
    cbar_3d = fig.colorbar( sm, ax=ax, shrink=0.5, aspect=15, pad=-0.1, cax=cbar_ax_3d, orientation="horizontal",)
...
if __name__ == "__main__":
    ...
    profile_2 = {key: (profile_2[key].entries / capa if key == outputs[-1] else profile_2[key].entries) for key in outputs}
    ...
    input_data = np.stack(
        [
            np.column_stack([prof['Current [A]'], prof['Terminal voltage [V]'], prof['X-averaged cell temperature [C]'], prof['Discharge capacity [A.h]']])
    ...
    profile_1 = {key: (profile_1[key].entries / capa if key == outputs[-1] else profile_1[key].entries) for key in outputs}
    profile_2 = {key: (profile_2[key].entries / capa if key == outputs[-1] else profile_2[key].entries) for key in outputs}
    # Combine into a list of profiles
    profiles = [profile_0, profile_1, profile_2]
    ...
    y_labels = ["SoC [·]", "X-Avg. Temperature [°C]", "Terminal Voltage [V]", "Terminal Current [A]"]
    ...
    for row, key in enumerate(reversed(outputs)):  # reversed to match bottom to top
        ...
        for idx, prof in enumerate(profiles):
            axs[row, 0].plot(time, prof[key], label=f'Profile {idx}', color=colors[idx], linestyle=linestyles[idx])
        ...
        for idx, prof in enumerate(profiles):
            ...
            axs[row, 1].plot(slice_time, prof[key][slice_range], label=f'Profile {idx}', color=colors[idx], linestyle=linestyles[idx])
        if row == 3:
